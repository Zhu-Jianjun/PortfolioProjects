...
#1: League associated with Country 
SELECT * FROM country;
...
#2: Teams associated with League 
SHOW COLUMNS FROM Team;
SHOW COLUMNS FROM league;
SHOW COLUMNS FROM matches;

SELECT Team.team_long_name, Team.team_short_name, league.name AS league_name, country.name AS country_name
FROM matches
INNER JOIN Team
...
SELECT * FROM player_attributes; 
/*
to potentially improve the dataset, a column 'positions' should be considered/added in player_attributes, s.t the player info. could be comparative based on the positions.
e.g., forward, center_forward, fullback, goalkeeper..., 
CASE
...
#6: detailed match-by-match info. 
SELECT
    m.match_api_id,
...
#7: Goals scored by home team vs away team Season on Season
SELECT
    ht_table.season,
    ht_table.leaguename,
...
#9: Matches won by Teams Season on Season
SELECT 
...
#10. Most Successful Teams (by win_percentage) compared for all seasons
SELECT 
    team,
